...
class ExportCommand(apper.Fusion360CommandBase):
    ...
    def on_execute(self, command: adsk.core.Command, inputs: adsk.core.CommandInputs, args, input_values):
        ...
        output_folder_path = output_folder + root_folder.name + os.path.sep

        # Create the base folder for this output if doesn't exist
        if not os.path.exists(output_folder_path):
            os.makedirs(output_folder_path)

        export_folder(root_folder, output_folder_path, file_types, write_version, name_option, folder_preserve)
        shutil.make_archive(output_folder + os.path.sep + root_folder.name + datetime.now().strftime("%Y_%m_%d_%H_%M_%S"), 'zip', root_dir = output_folder_path)
        ...
        if len(DUPED_FILES) > 0:
            ao.ui.messageBox(
                "The following files were duplicates: {}".format(
                    DUPED_FILES
                )
            )
